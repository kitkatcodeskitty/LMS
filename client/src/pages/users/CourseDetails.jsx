...
const CourseDetails = () => {
  ...
  const { allCourses, calculateRating,calculateChapaterTime,calculateCourseDuration,calculateNoOfLectures,currency,backendUrl, userData,getToken } = useContext(AppContext)

  const fetchCourseData = async () => {
    try {
      const {data} = await fetch(backendUrl + '/api/courses' + id)

      if (data.success) {
        setCourseData(data.course);
        setIsAlreadyEnrolled(data.isAlreadyEnrolled);
      }else {
            ...
            {courseData.courseContent.map((chapter,index)=>(
              <div key={index} className='border border-grey-300 bg-white mb-2 rounded' >
                ...
                <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={()=> toggleSection(index)}>
                  ...
                  <p className='text-sm md:text-default'>{chapter.chapterContent.length} Lectures - {calculateChapaterTime(chapter)}</p>
                </div>
